...
const withinExperiment = (experimentName: string, selector: string) => `[experiment-name="${experimentName}"] ${selector}`
...
export class Tester {
  ...
  public async getByText(text: string, containerSelector = '') {
    const element = await this.page.$(`${containerSelector} *::-p-text(${text})`)
  ...
  public async getInputValue(inputSelector: string) {
    return this.page.$eval(inputSelector, (input) => (input as HTMLInputElement).value)
  }

  public async isButtonDisabled(buttonSelector: string) {
    return this.page.$eval(buttonSelector, button => (button as HTMLButtonElement).disabled)
  }
